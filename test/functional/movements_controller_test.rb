...
class MovementsControllerTest < ActionController::TestCase
  ...
  test "should create movement" do
    assert_difference('Movement.count') do
      post :create, :movement => { :articleCode => @movement.articleCode, :movementDate => @movement.movementDate, :quantity => @movement.quantity, :turnover => @movement.turnover }
    end
  ...
  test "should update movement" do
    put :update, :id => @movement, :movement => { :articleCode => @movement.articleCode, :movementDate => @movement.movementDate, :quantity => @movement.quantity, :turnover => @movement.turnover }
    assert_redirected_to movement_path(assigns(:movement))
  end
